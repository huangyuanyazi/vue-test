<template>
	<f7-page :no-navbar="enableNobar" :no-toolbar="enableNobar" name="patent_search" id='patent_search'>
		<!--:clear="true"-->
		...
		<f7-card id="hot">
			...
			<f7-card-content>
				<f7-chip :text="hotSearch.hotSearchItem"
						 v-for="hotSearch in hotSearchAll"
...
<script>
    import hotSearch from '../../db/hotSearch'
    export default{
        name: 'TqPatentSearch',
        data(){
            return {
                        ...
                        $$.ajax({
                            ...
                            success: function (data) {
                                ...
                                for (var i = 0; i < data.length; i++) {
                                    if (data[i].searchContent.toLowerCase().indexOf(query.toLowerCase()) >= 0)
                                        results.push(data[i]);
                                }
                                //隐藏加载样式
