 ...
 */
 let ModelValidator = class {
	...
	static match(validation, field, value, option, label, ruleOptMsg) {
		...
		if (!_.isRegExp(pattern)) throw new Error('验证参数不是有效的正则对象');
	...
	static json(validation, field, value, option, label, ruleOptMsg) {
		...
		if (type === 'array') {
			if (!_.isArray(value)) return [label + '不是有效的JSON数组格式'];
		}
	};
	/**
	 * 验证是否为有效的URL
	...
	static _remote(method, validation, field, value, option) {
		...
		return axios
			.post(this.backend, qs.stringify(data), {
				headers: {'_rule_': encodeURI(JSON.stringify(helper.getYiiRules(rule)))},
