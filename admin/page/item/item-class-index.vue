<template>
    <div class="c-item-category-index">
        ...
        <div class="c-main">
            <el-tree :data="classes" node-key="id" :props="defaultProps" :default-expand-all="true" :renderContent="renderContent" class="c-tree"></el-tree>
        </div>
    </div>
</template>
...
<script>
    export default {
        ...
        methods: {
            ...
            create () {
                this.$router.push('/item/item-class/create')
            },
            modify (event, node) {
                event.stopPropagation() //阻止事件冒泡，防止点击时展开或收缩节点
                let id = node.data.id
                console.log("修改节点：" + id)
                this.$router.push({path: '/item/item-class/update', query: { id }})
            },
